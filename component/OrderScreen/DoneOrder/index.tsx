...
const DoneIndexPage = ({onPress, ...props}: ExtendedBoxProps) => {
  ...
  function alert(arg0: string) {
    console.log('log', arg0);
  }
  const handleNavigateToOrderDetail = (orderData) => {
    ...
    <>
      <SwipeListView
        ...
        renderItem={data => (
          <TouchableOpacity
            onPress={() => {
              const orderData = data.item; // 通过key找到特定订单
              handleNavigateToOrderDetail(orderData);
            }}
          >
            <Box
              key={data.item.key}
        ...
        renderHiddenItem={v => (
          <Stack direction='row' ml={'1.5'} mr={'1.5'} mt={'3'}>
            <VStack>
              ...
              >
                <Icon as={<FontIcon name='trash-o' />} color='white' />
              </Pressable>
              <Pressable
                width={Dimensions.get('screen').width * 0.5}
                px={5}
